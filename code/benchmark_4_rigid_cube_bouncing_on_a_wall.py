...
class RigidFluidCoupling(Application):
    ...
    def add_user_options(self, group):
        group.add_argument("--coeff-of-restitution",
                           action="store",
                           type=float,
                           dest="coeff_of_rest",
                           default=1.0,
                           help="Coefficient of resitution (default to 1.0)")

    def consume_user_options(self):
        self.coeff_of_rest = self.options.coeff_of_rest
    ...
    def create_particles(self):
        ...
        # setup the damping aka coeff of restitution
        coeff_of_rest = np.ones(body.nb[0]*body.total_no_bodies[0],
                                dtype=float) * self.coeff_of_rest
        body.add_constant('coeff_of_rest', coeff_of_rest)
        setup_damping_coefficient(body, [body], boundaries=[tank])
